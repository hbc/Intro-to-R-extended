...
A variable is a symbolic name for (or reference to) information. Variables in computer programming are analogous to "bucket", where information can be maintained and referenced. On the outside of the bucket is a name. When referring to the bucket, we use the name of the bucket, not the data stored in the bucket.
...
- What happens if we change the value of what's inside `x`, what happens to `number`?
...
Each element of the vector contains a single numeric value, and three values will be combined together using `c()` (the combine function). All the values are put within the parentheses and separated with a comma.
...
A `matrix` in R is a colllection of vectors of same length _and_ identical datatype. Vectors can be combined as columns in the matrix or by row. Usually matrices are numeric and used in various computational algorithms to serve as a checkpoint. For example, if input data is not of identical datatype, the `matrix()` function will throw an error and stop any downstream code execution.

`data.frame` is the _de facto_ data structure for most tabular data and what we use for statistics and plotting. A `data.frame` is a collection of vectors of identical lengths. Each vector represents a column, and each vector can be of a different data type (e.g.,
